<header class="bg-white dark:bg-gray-800 shadow-md dark:shadow-white h-16 w-full flex justify-between items-center py-2 dark:text-white border-b-2 dark:border-white">
    <div class="flex items-center">
        ...
        <a href="<?php echo e(route('home')); ?>" class="hidden md:flex items-center justify-center" aria-label="Home">
            <img src="<?php echo e(asset(auth()->user()->school->logoURL ?? config('app.logo'))); ?>" alt="" class="rounded-full w-12 h-12 border border-gray-200 shadow-lg">
    ...
    <div class="flex justify-evenly items-center gap-6 px-5 h-full" x-data="{'dropDownOpen': false, 'darkMode' : $persist(false), 'fullScreen' : $persist(false) }">
        ...
        <button class="h-full flex items-center gap-2"  @click="dropDownOpen = !dropDownOpen">
            ...
            <div class="flex items-center h-full">
                <img src="<?php echo e(auth()->user()->profile_photo_url); ?>" alt="" class="rounded-full w-10 h-10 border border-gray-200 shadow-md">
                <p class="hidden lg:block px-2"  ><?php echo e(auth()->user()->name); ?></p>
            </div>
        ...
        <div class="absolute bg-blue-700 dark:bg-gray-800 top-16 w-5/6 border  md:w-2/6 lg:w-1/5 shadow-md right-2 flex flex-col items-center justify-center rounded p-4 text-white " x-show="dropDownOpen" x-transition style="display: none" @click.outside="dropDownOpen = false">
            <img src="<?php echo e(auth()->user()->profile_photo_url); ?>" alt="" class="rounded-full w-20 h-20 border border-gray-200 shadow-md">
            <h2 class="text-lg  font-bold"><?php echo e(auth()->user()->name); ?></h2>
            <p class="text-center">
                <!--[if BLOCK]><![endif]--><?php if(isset(auth()->user()->school)): ?>
                    Academic year: <?php echo e(auth()->user()->school->academicYear?->name); ?> <br>
                    Semester: <?php echo e(auth()->user()->school->semester?->name); ?>

                <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
            </p>
            <form action="<?php echo e(route('logout')); ?>" class="w-full" method="POST">
                <?php echo csrf_field(); ?>
...
</header><?php /**PATH /home/schrodingercat/Desktop/schoolManagement/skuul/resources/views/livewire/layouts/header.blade.php ENDPATH**/ ?>